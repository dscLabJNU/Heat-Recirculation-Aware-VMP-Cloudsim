...
public class SABA {
    ...
    static int serverNum=0;//开启服务器数量
    ...
    public static int[] initTaskAllocation() {
        ...
        serverNum = serverNum(taskNum);//启动的服务器数量
        for (int i = 0; i < serverNum; i++) {
            taskAllocation[i] = (int) (Math.random() * 9 + 1);
        ...
        while (true) {                           //校正任务数量
            int checkTask = (int) (Math.random() * serverNum);
            if (taskAllocation[checkTask] + remainTask >= 0 && taskAllocation[checkTask] + remainTask <= 40) {
                taskAllocation[checkTask] += remainTask;
                break;
            }else if(remainTask>0){
                remainTask=remainTask-(40-taskAllocation[checkTask]);
                taskAllocation[checkTask]=40;
            }else if(remainTask<0){
                remainTask=remainTask+taskAllocation[checkTask];
                taskAllocation[checkTask]=0;
            }
        }
        boolean flag = true;
        while (flag) {
...
//        System.out.println("最终实际分配任务总数："+totalTask);
    ...
    private static int serverNum(int taskNum) {
        ...
        if (taskNum <= 1300) {
            serverNum = (int) Math.ceil(taskNum / 0.65 / 40);
        }
        return serverNum;
    }
